...
__bit gsm_sendandcheck(u8 *cmd, u8 retry, u8 delay, u8 *display) {
  ...
  while (!gui_lenh_thanh_cong && total_try_time_out && !nosim) {
    ...
    if (GPS_time)
      LCD_guigio(0xc0, "  GPS  ", hour, minute, second, flip_pulse);
    else if (!eep_gpson)
      LCD_guigio(0xc0, " ASIA  ", hour, minute, second, flip_pulse);
    else
      LCD_guigio(0xc0, "   DS  ", hour, minute, second, flip_pulse);
    if (skip_gsm_cmd) {
      skip_gsm_cmd = 0;
      return 0;
    }
...
__bit send_sms(__bit chinh) {
  if (lenh_sms[0] && !lenh_sms[3]) {
    gsm_sendandcheck("\032", 3, 1, "TK<1000 K BAOCAO");
    return 0;
  }
  send_gsm_cmd("AT+CMGS=\"");
  if (chinh)
    send_gsm_cmd(phone_chinh);
  else
...
__bit kiemtrasodienthoai() {
  ...
  switch (nha_mang) {
  case VIETTEL:
    return gsm_sendandcheck("AT+CUSD=1,\"*101#\",\r", 3, 30,
                            "  KIEM TRA SDT   ");
  case VINAPHONE:
    return gsm_sendandcheck("AT+CUSD=1,\"*110#\",\r", 3, 30,
                            "  KIEM TRA SDT   ");
  case MOBIFONE:
...
void kiemtratinhieu() {
  ...
  if (gsm_sendandcheck("AT+CSQ\r", 15, 1, " SONG ")) {
    ...
    signal = 0;
    while (lenh_sms[i] != ',')
      signal = lenh_sms[i++] - 48 + signal * 10;
  }
}
...
// Configure ATGM336H GPS module
__bit gps_configure_atgm336h() {
  LCD_guilenh(0x80);
  LCD_guichuoi("CONFIG GPS ATGM ");

  // Step 1: Change baud rate to 38400
  send_gsm_cmd("$PCAS01,3*1F\r\n");
  delay_ms(500); // Wait for command to process

  // Step 2: Set 1Hz update rate
  send_gsm_cmd("$PCAS02,1000*2E\r\n");
  delay_ms(100);

  // Step 3: Enable RMC only
...
__bit gsm_thietlapnhantin1() {
  if (!gsm_pw || !sms_on)
    return 0;
  if (!gsm_sendandcheck("AT\r", 15, 1, "THIET LAP TNHAN "))
    return 0;
              ...
              if (have_time == CFUN) {
                ...
                if (phay > 2) {
                  if (date_str[i + 1] == ',')
                    ...
                    hour = (date_str[i++] - '0') * 10 + (date_str[i++] - '0');
                  if (date_str[++i + 1] == ',')
                    minute = (date_str[i++] - '0');
                  else
                    minute = (date_str[i++] - '0') * 10 + (date_str[i++] - '0');
                  if (date_str[++i + 1] == ',')
                    second = (date_str[i++] - '0');
                  else
                    second = (date_str[i++] - '0') * 10 + (date_str[i++] - '0');
...
void gsm_serial_interrupt() __interrupt(gsm_SERIAL_INT) __using(SERIAL_MEM) {
  if (gsm_RI) {
    ...
    if ((gsm_receive_buf[gsm_receive_pointer] == 'N' &&
         gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == 'I' &&
         gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == ' ' &&
         gsm_receive_buf[(gsm_receive_pointer + 10) % 13] == 'T' &&
         gsm_receive_buf[(gsm_receive_pointer + 9) % 13] == 'O' &&
         gsm_receive_buf[(gsm_receive_pointer + 8) % 13] == 'N')) {
    ...
    case CSQ:
      ...
      if (SBUF == ' ' &&
          gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == ':')
        sms_index = 0;
      if (SBUF == ',')
    ...
    case CLK:
      lenh_sms[sms_index++] = SBUF;
      if (SBUF == 'K' &&
          gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == 'L' &&
          gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == 'C')
        sms_index = 0;
      if (SBUF == '\r' &&
          gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == '"')
        sms_index = gsm_serial_cmd = 0;
    ...
    case NORMAL:
      if (SBUF == '>') {
      ...
      } else if ((gsm_receive_buf[gsm_receive_pointer] == 'G' &&
                  gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == 'N' &&
                  gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == 'I' &&
                  gsm_receive_buf[(gsm_receive_pointer + 10) % 13] == 'R')) {

        send_gsm_cmd("ATH\r");

        /*SMS buoc 1: khi co tin nhan toi se co lenh CMTI bat co tin nhan moi
           new_message = 1 khi co tin nhan moi thi gui lenh CMGL ALL de liet ke
           tat ca tin nhan toi*/
      } else if ((gsm_receive_buf[gsm_receive_pointer] == ':' &&
                  gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == 'I' &&
                  gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == 'T' &&
                  gsm_receive_buf[(gsm_receive_pointer + 10) % 13] == 'M' &&
                  gsm_receive_buf[(gsm_receive_pointer + 9) % 13] == 'C' &&
                  gsm_receive_buf[(gsm_receive_pointer + 8) % 13] == '+')) {

        co_tin_nhan_moi = 1;

        /*SMS buoc 2: sau khi kiem duoc CMGL thi chuyen qua tim kiem so dien
           thoai phu hop neu nhu da nhan duoc tin nhan can xu ly thi khong doc
           tin nhan khac tiep tuc*/
      } else if ((gsm_receive_buf[gsm_receive_pointer] == ' ' &&
                  gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == ':' &&
                  gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == 'L' &&
                  gsm_receive_buf[(gsm_receive_pointer + 10) % 13] == 'G' &&
                  gsm_receive_buf[(gsm_receive_pointer + 9) % 13] == 'M' &&
                  gsm_receive_buf[(gsm_receive_pointer + 8) % 13] == 'C' &&
                  gsm_receive_buf[(gsm_receive_pointer + 7) % 13] == '+') ||
                 (gsm_receive_buf[gsm_receive_pointer] == ' ' &&
                  gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == ':' &&
                  gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == 'P' &&
                  gsm_receive_buf[(gsm_receive_pointer + 10) % 13] == 'I' &&
                  gsm_receive_buf[(gsm_receive_pointer + 9) % 13] == 'L' &&
                  gsm_receive_buf[(gsm_receive_pointer + 8) % 13] == 'C' &&
                  gsm_receive_buf[(gsm_receive_pointer + 7) % 13] == '+')) {
        co_cuoc_goi_toi =
            gsm_receive_buf[(gsm_receive_pointer + 9) % 13] == 'L';
        if (!co_cuoc_goi_toi && sms_dang_xu_ly)
          break;
      ...
      } else if (gsm_receive_buf[gsm_receive_pointer] == 'K' &&
                 gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == 'O') {
        if (gsm_serial_cmd == CLK)
          sms_index = 0;
        ...
        // GPS Module Detection: Check for $GNRMC sentence
                 ...
                 gsm_receive_buf[(gsm_receive_pointer + 7) % 13] == '$') {
        // Detected $GNRMC sentence - this is a GPS module
        // debug
        // send_gsm_cmd("gps ok");

        gps_module_atgm = 1;
        gnrmc_field_index = 0; // Start parsing from field 0
        gnrmc_char_index = 0;
        gsm_serial_cmd = GNRMC_PARSE;

      } else if (gsm_receive_buf[gsm_receive_pointer] == 'T' &&
                 gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == 'O' &&
    ...
    case CFUN1:
      if (gsm_receive_buf[gsm_receive_pointer] == ' ' &&
          gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == ':' &&
          gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == 'Z' &&
          gsm_receive_buf[(gsm_receive_pointer + 10) % 13] == 'T' &&
          gsm_receive_buf[(gsm_receive_pointer + 9) % 13] == 'T' &&
          gsm_receive_buf[(gsm_receive_pointer + 8) % 13] == 'U' &&
          gsm_receive_buf[(gsm_receive_pointer + 7) % 13] == 'S' &&
          gsm_receive_buf[(gsm_receive_pointer + 6) % 13] == 'P' &&
    ...
    case CFUN:
      ...
      break;
    case PHONE:
      /*SMS buoc 4: khi da tim thay +84 thi se so sanh so dien thoai xem co
         trung khop khong neu khong trung khop thi quay lai tim CMGL hoac OK*/
      if (phone_header) {
        if (!sms_index) {
          if (SBUF == '\r') {
            if (co_cuoc_goi_toi) {
        ...
        } else if (sms_index == PHONE_LENGTH &&
                   gsm_SBUF == '"') { /*SMS buoc 5: neu tat ca chu so dt deu
                                         trung chuyen qua tim lenh */
          sms_index = 0;

        } // neu co mot chu so khong trung thi reset sms_index phone_header va
          // chuyen ve tim CMGL hoac OK
        else {
          // if(phone_chinh[sms_index++] != gsm_SBUF)gsm_serial_cmd = NORMAL;
          if (!phone_chinh_so_sanh_that_bai &&
              phone_chinh[sms_index] != gsm_SBUF)
            phone_chinh_so_sanh_that_bai = 1;
          if (!bat_phone_phu || (!phone_phu_so_sanh_that_bai &&
                                 eep_phonephu[sms_index] != gsm_SBUF))
            phone_phu_so_sanh_that_bai = 1;
          sms_index++;
          if (phone_chinh_so_sanh_that_bai && phone_phu_so_sanh_that_bai)
            gsm_serial_cmd = NORMAL;
        }

      } /*SMS buoc 3: sau khi thay CMGL thi se vao day luc nay se tim kiem +84
           la dau so chuan neu khong tim duoc truoc khi co ky tu xuong dong thi
           quay ve tim CMGL hoac OK*/
      else {
        if ((co_cuoc_goi_toi && gsm_receive_buf[gsm_receive_pointer] == '0' &&
             gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == '"') ||
            (gsm_receive_buf[gsm_receive_pointer] == '4' &&
             gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == '8' &&
             gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == '+' &&
             gsm_receive_buf[(gsm_receive_pointer + 10) % 13] == '"')) {
          phone_header = 1;
          sms_index = 1;
        }
        if (SBUF == '\r')
      ...
      break;
    case CMD: /*SMS buoc 7: tim duoc pin chinh xac tien hanh lay lenh trong tin
                 nhan khi gap ; hoac xuong dong hoac het 160 ky tu tin nhan thi
                 chuyen qua xu ly lenh thong bao gui lenh thanh cong*/
      if (pin_chinh_xac) {
      ...
      } else { /*SMS buoc 6: tim xem co ma pin trung khop khong neu khong co
                  truoc khi gap ky tu xuong dong thi quay ve NORMAL*/
        pin_chinh_xac =
            gsm_receive_buf[gsm_receive_pointer] == ',' &&
            gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == 'C' &&
            gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == 'U' &&
            gsm_receive_buf[(gsm_receive_pointer + 10) % 13] == 'D' &&
            gsm_receive_buf[(gsm_receive_pointer + 9) % 13] == 'P' &&
            gsm_receive_buf[(gsm_receive_pointer + 8) % 13] == '*';
        if (SBUF == '\r') {
          sms_index = 1;
          gsm_serial_cmd = NORMAL;
    ...
    case CMGS:
      if (SBUF == '>') {
        gsm_serial_cmd = NORMAL;
        gui_lenh_thanh_cong = 1;
      } else if (gsm_receive_buf[gsm_receive_pointer] == 'R' &&
                 gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == 'R' &&
                 gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == 'E')
        connect = 0;
      break;
    case CUSD:
      if (have_cusd) {
        if ((((nha_mang == VINAPHONE || nha_mang == MOBIFONE) && SBUF != ' ') ||
             ((nha_mang == VIETTEL || nha_mang == VIETNAM) && SBUF != 'd')) &&
            sms_index < 160) {
          if (SBUF != '.')
            lenh_sms[sms_index++] = SBUF;
        } else {
          ...
          gui_lenh_thanh_cong = 1;
                  ...
                  gsm_receive_buf[gsm_receive_pointer] == '=' &&
                  gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == 'h' &&
                  gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == 'n' &&
                  gsm_receive_buf[(gsm_receive_pointer + 10) % 13] == 'i' &&
                  gsm_receive_buf[(gsm_receive_pointer + 9) % 13] == 'h' &&
                  gsm_receive_buf[(gsm_receive_pointer + 8) % 13] == 'c') ||
                 (nha_mang == VIETTEL &&
                  gsm_receive_buf[gsm_receive_pointer] == ' ' &&
                  gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == ':' &&
                  gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == 'G' &&
                  gsm_receive_buf[(gsm_receive_pointer + 10) % 13] == 'K' &&
                  gsm_receive_buf[(gsm_receive_pointer + 9) % 13] == 'T' &&
                  gsm_receive_buf[(gsm_receive_pointer + 8) % 13] == ' ') ||
                 (nha_mang == MOBIFONE &&
                  gsm_receive_buf[gsm_receive_pointer] == ' ' &&
                  gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == 'C' &&
                  gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == 'K' &&
                  gsm_receive_buf[(gsm_receive_pointer + 10) % 13] == 'T' &&
                  gsm_receive_buf[(gsm_receive_pointer + 9) % 13] == ' ' &&
                  gsm_receive_buf[(gsm_receive_pointer + 8) % 13] == '.') ||
                 // (nha_mang == MOBIFONE &&
                 // gsm_receive_buf[gsm_receive_pointer]==':' &&
                 //     gsm_receive_buf[(gsm_receive_pointer+12)%13] =='C' &&
    ...
    case SDT:
      if (have_cusd) {
        if ((((nha_mang == VINAPHONE || nha_mang == VIETNAM) && SBUF != '\n') ||
             (nha_mang == MOBIFONE && SBUF != '"') ||
             (nha_mang == VIETTEL && SBUF != '.')) &&
            sms_index < 10)
          if (sms_index < 9)
            phone[1 + sms_index++] = SBUF;
          else
            sms_index++;
        else {
          have_quote = have_cusd = 0;
          ...
          sms_index = 0;
                  ...
                  gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == ')') ||
                 (nha_mang == VIETNAM &&
                  gsm_receive_buf[gsm_receive_pointer] == '0' &&
                  gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == ' ') ||
                 ((nha_mang == MOBIFONE || nha_mang == VIETTEL) &&
                  gsm_receive_buf[gsm_receive_pointer] == '4' &&
                  gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == '8'))
        have_cusd = 1;
      else if ((SBUF == '5' &&
                gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == '1' &&
                gsm_receive_buf[(gsm_receive_pointer + 11) % 13] == ' ' &&
                gsm_receive_buf[(gsm_receive_pointer + 10) % 13] == ',') ||
               (SBUF == 'R' &&
                gsm_receive_buf[(gsm_receive_pointer + 12) % 13] == 'R' &&
